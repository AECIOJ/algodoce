...
def rota(path, methods=None, endpoint=None, defaults=None):
    ...
    No import do módulo apenas registra a view no motor; o bp é montado
    depois por `montar_blueprint`. O `endpoint` default é o nome da função.
    """
...
def _page_single(mod):
    """True quando o módulo NÃO é crud (página única via do_page)."""
    ...
    if not callable(get):
        return False
    if get('type') is not None:
        return get('type') != 'crud'
    props = get('props') or {}
...
def montar_blueprint(mod, slug=None, url_prefix=None, login=True, label=None):
    ...
    for r in custom:
        ...
        bp.add_url_rule(r['path'], r['endpoint'], r['func'], **kwargs)
    ...
    try:
        from ajsystem.core.showcase import generated_routes as _showcase_routes
    except ImportError:
        _showcase_routes = None
    if _showcase_routes is not None:
        for rule, endpoint, func, methods in _showcase_routes(mod):
            if endpoint in custom_names:
                continue
    ...
    if _page_single(mod) and 'list' not in custom_names:
        from ajsystem.core.do_page import do_page
        bp.add_url_rule('/', 'list', lambda mod=mod: do_page(mod),
...
def _registrar_rotas_automaticas(app):
    ...
    if not has_sistema:
        ...
        def _site_sistema():
            ...
            try:
                url = pagina_home(APP.module('system'))
            except Exception:
                url = None
            if url:
