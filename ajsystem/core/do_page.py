...
from ajsystem.core.adapter import get_markdown_loader
from ajsystem.core.cart import cart_context
from ajsystem.core.showcase import showcase_context

_ROUTES_PREFIX = None  # resolvido via adapter.ROUTES_BASE (layout do host)


def _nome_rotas(mod):
    from ajsystem.core.adapter import ROUTES_BASE
    prefix = _ROUTES_PREFIX or (ROUTES_BASE + '.')
    nome = getattr(mod, '__name__', '')
    if nome.startswith(prefix):
        return nome[len(prefix):]
    return nome
