...
def do_form(form_spec, id=None, extra_ctx=None, instance=None):
    ...
    for _s in form._resolved_sessions:
        _group_by = _s.get('group_by')
        _is_grouped_table = isinstance(_s.get('table'), dict) and bool(_s['table'].get('group_by'))
        if not (_s.get('query') or _is_grouped_table):
            continue
        if _group_by:
            _items = order_items(
            ...
            _gf = next((f for f in _s.get('fields', []) if f.name == _group_by), None)
