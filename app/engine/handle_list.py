...
from app.list import List, Field, build_field_config, build_filter_config, build_field_context, get_field
...
def _resolve_model(entity_name: str):
    from app.list import MODEL_MAP
...
def render_list(entity_name: str, module_name: str, data=None, **extra):
    ...
    if entity_name not in entidades:
        raise KeyError(f"Entidade '{entity_name}' não definida em {module_name}")

    model = _resolve_model(entity_name)

    field_configs = _resolve_cols(lista.get('colunas', [entity_name]), entidades)
    ...
    list_obj = List(
        ...
        edit_endpoint=lista.get('edit_endpoint'),
    ...
    detail_data = None
    if 'detalhe' in lista:
        det_configs = _resolve_cols(lista['detalhe'], entidades)
    ...
    new_endpoint = lista.get('new_endpoint')
    ...
    template = lista.get('template') or "components/page_list_simple.html"
