...
def list():
    ...
    return render_template(
        "orcamentos/list.html", orders=quotes, fields=QUOTES_FIELDS, ctx=ctx,
...
def detail(id):
    ...
    return render_template("orcamentos/detail.html", quote=quote, nav=nav,
...
def edit(id):
    ...
    return render_template(
        "orcamentos/form.html", quote=quote, products=products, nav=nav,
...
def new():
    ...
    carteiras = Carteira.query.order_by(Carteira.nome).all()
    return render_template(
        "orcamentos/form.html", quote=None, products=products,
        tipos_evento=tipos_evento, clients=clients,
        QUOTE_STATUS=QUOTE_STATUS, FORMINHAS=FORMINHAS,
        carteiras=carteiras,
    )
...
def print_quote(id):
    quote = Quote.query.get_or_404(id)
    return render_template("orcamentos/print.html", quote=quote, FORMINHAS=FORMINHAS)
