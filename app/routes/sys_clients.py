...
def new():
    if request.method == "POST":
        ...
        if cpf and cnpj:
            ...
            return render_template("sys_contas/form.html", conta=None, TIPO_CONTA=TIPO_CONTA)
...
def edit(id):
    ...
    if request.method == "POST":
        ...
        if cpf and cnpj:
            flash("Preencha apenas CPF ou CNPJ, não ambos.", "warning")
            return render_template("sys_contas/form.html", conta=conta, nav=nav, orders=orders, ORDER_STATUS=ORDER_STATUS, TIPO_CONTA=TIPO_CONTA)
        if cpf and not _cpf_valido(cpf):
            flash("CPF inválido.", "warning")
            return render_template("sys_contas/form.html", conta=conta, nav=nav, orders=orders, ORDER_STATUS=ORDER_STATUS, TIPO_CONTA=TIPO_CONTA)
        if cnpj and not _cnpj_valido(cnpj):
