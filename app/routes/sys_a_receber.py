...
CONTAS_A_RECEBER_FIELDS = [
    ...
    Field(name='pedido_id', label='Pedido', width=6, pos=-1),
...
bp = Blueprint("a_receber", __name__, url_prefix="/contas-a-receber")
...
def detail(id):
    ...
    return render_template(
        "sys_a_receber/detalhes.html", t=transacao,
...
def new():
    ...
    back_url = url_for("orders.edit", id=order.id) if order else url_for("a_receber.list")
