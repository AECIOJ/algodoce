...
from app.ajsystem.defs.data import fk_target_model, resolve_max_width
from app.ajsystem.defs.tags import parse_tag, resolve_tag
...
from app.ajsystem.core.query import group_items, order_items
...
def _build_lookup(form, extra_lookup=None, instance=None):
    ...
    def fill(f, src_model, _union=True):
        ...
        if isinstance(resolved, dict) and resolved.get('query'):
            ...
            return
        if has_when:
            from app.ajsystem.defs.data import apply_lookup_when
