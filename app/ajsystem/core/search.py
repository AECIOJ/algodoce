...
def _coerce_scalar(v):
    from app.ajsystem.defs.data import _when_value
    return _when_value(v)


def _coerce_list(s):
    from app.ajsystem.defs.data import _when_in_list
...
def _py_val(row, path, entity):
    ...
    from app.ajsystem.core.utils import calc_value, deep_attr
    if '.' in path:
...
def _fmt_cell(value, cfg):
    from app.ajsystem.core.utils import fmt_money, fmt_date, fmt_datetime
    if value is None:
        return '—'
...
def run_search(spec, params):
    ...
    from app.ajsystem.core.query import order_items
    ...
    if order:
        from app.ajsystem.core.utils import deep_attr as _da
