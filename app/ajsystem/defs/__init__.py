...
`Form`, `List`, `Editor`), a tipagem do config do app (`App`, `Module`,
`MenuItem`, `Tema` em `config`), tabelas de tipos, constantes e resolução de
campos. Não depende de renderização (`core/form`, `core/list`, `core/query`),
de request nem do motor (`core/auto`, `core/menu`).
