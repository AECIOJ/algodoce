...
def _resolve_child_entity(parent_model, entidade, col_names):
    """Infere a Entity-key do filho de uma sub-tabela a partir dos nomes de
    colunas: percorre as relações pai→filho e escolhe a cuja config de `Entity`
    pertencem os campos. Usada quando `table.columns` é informado sem `entity`."""
    col_names = [c for c in (col_names or []) if c]
    if parent_model is None or not entidade:
        return None
    try:
        relationships = list(parent_model.__mapper__.relationships.items())
    except Exception:
        return None
    candidates = []
    for name, rel in relationships:
        try:
            if rel.direction is MANYTOONE or rel.secondary is not None:
                continue
        ...
        for k in (child_model.__name__, child_model.__name__.lower(), snake):
            if k in entidade:
                ent_key = k
                break
        if ent_key is None:
            continue
        ent_fields = set(_entidade_fields(entidade.get(ent_key)))
        score = len(col_names) - len(set(col_names) - ent_fields)
        if score > 0:
            candidates.append((score, ent_key))
    if not candidates:
        return None
    ...
    def _build_sessions(self):
        ...
        for key, cfg in self.sessions.items():
            ...
            if isinstance(table_cfg, dict):
                table_entity = table_cfg.get('entity')
                table_entities = [table_entity] if table_entity else []
                if not table_entities and table_cfg.get('columns') and self.model is not None:
                    mod = importlib.import_module(self.module_name) if self.module_name else None
                    entidade = getattr(mod, 'Entity', {}) if mod else {}
                    _col_names = [
                        (list(i.items())[0][0] if isinstance(i, dict) else i)
                        for i in table_cfg['columns']
                    ]
                    table_entities = [_resolve_child_entity(self.model, entidade, _col_names)]
                    table_entities = [e for e in table_entities if e]
            else:
                table_entities = query_entities if query_entities is not None else (table_cfg if isinstance(table_cfg, list) else [])
            meta = {}
            ...
            if is_report:
                ...
                if table_cfg.get('group_by'):
                    group_by = table_cfg.get('group_by')
                if table_cfg.get('order_by'):
                    order_by = table_cfg.get('order_by')
            if (is_report or group_by) and group_by:
                for f in fields:
                    if f.name == group_by:
                        f.in_form = 0
            totals = cfg.get('totals')
            table_config = cfg.get('table')
            _field_map = {f.name: f for f in fields}
            if totals:
                _enriched = {}
                for _tname, _tspec in totals.items():
                    _f = _field_map.get(_tname)
                    if isinstance(_tspec, str):
                        _entry = {'fn': _tspec}
                    else:
                        _entry = dict(_tspec)
                    if _f and _f.calc:
                        _entry['calc'] = _f.calc
                    if _f and _f.currency and 'currency' not in _entry:
                        _entry['currency'] = _f.currency
                    _enriched[_tname] = _entry
                totals = _enriched
            # `table.columns`: colunas exibidas (ordem + `agg` por-linha, declarativo).
            # Cada nome é resolvido pelos Field do filho (já herdados da Entity da
            # página); a lista de strings e/ou dicts `{name: {agg}}` define a ordem.
            _columns = None
            _agg_map = None
            if isinstance(table_config, dict) and table_config.get('columns'):
                _columns = []
                _agg_map = {}
                for _item in table_config['columns']:
                    if isinstance(_item, dict):
                        _fname, _fcfg = next(iter(_item.items()))
                        _f = _field_map.get(_fname)
                        if _f is None:
                            continue
                        if isinstance(_fcfg, dict) and _fcfg.get('agg'):
                            _agg_map[_fname] = _fcfg['agg']
                        _columns.append(_f)
                    else:
                        _f = _field_map.get(_item)
                        if _f is not None:
                            _columns.append(_f)
            # `table.total`: lista de nomes de campos -> soma vertical no rodapé,
            # enriquecido com `calc`/`currency` dos Field do filho.
            if isinstance(table_config, dict):
                _ttotal = table_config.get('total')
                if isinstance(_ttotal, list):
                    _tfields = {}
                    for _fname in _ttotal:
                        _f = _field_map.get(_fname)
                        _entry = {'fn': 'sum'}
                        if _f and _f.calc:
                            _entry['calc'] = _f.calc
                        if _f and _f.currency:
                            _entry['currency'] = _f.currency
                        _tfields[_fname] = _entry
                    table_config = dict(table_config)
                    table_config['total'] = _tfields
            if group_by and group_totals is None and isinstance(table_config, dict) and isinstance(table_config.get('total'), dict):
                group_totals = {}
                for _fname, _entry in table_config['total'].items():
                    _entry = _entry if isinstance(_entry, dict) else {}
                    _spec = {'sum': _fname}
                    if _entry.get('currency'):
                        _spec['currency'] = True
                    group_totals[_fname] = _spec
            result.append({
                'attr': rel,
                ...
                'readonly': True if (is_report or group_by) else (cfg.get('readonly', False) or bool(meta.get('readonly', False))),
