...
Camada de dados do framework: `Entity`, `MODEL_MAP` e os helpers de
resolução/construção de campos. As dataclasses `Field`/`Query` vivem em
`defs.fields`/`defs.query`. Não depende de renderização (`list`),
formulário (`form`) nem relatórios (`report`).

Dependências: apenas as definições `config.fields` + `utils`.
"""
import importlib
